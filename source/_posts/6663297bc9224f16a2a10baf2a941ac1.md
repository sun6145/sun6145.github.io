...
# 还提供特定版本（待固定）：例如：或 docker pull jenkins/jenkins:<version>-<jdk>
...
![](/resources/e67ccacbeaa34a51a0b37d1fa23f3a3e.png)

![](/resources/d424632854a84886a2bc0c400ec801c6.png)
...
Gitea 默认认为 SSH 使用的是 **22 端口**，但我们容器中实际映射的是 **2222**，因此需要额外配置。

如果不进行配置，那么在页面中提供的 ssh 地址是不带端口的，这样默认就是使用 22 端口，会导致连接不了，
...
1. 修改 Gitea 的 app.ini 配置
   ...
   # 用户克隆仓库时直接指定端口：
   git clone ssh://git@git.fulsun.local:2222/username/repo.git
   ```
...
可选：服务器端口转发（需 root 权限）
如果希望隐藏端口，将主机的 22 端口转发到 2222：
...
```sh
git clone ssh://git@192.168.61.10:2222/fulsun/demo.git
```
